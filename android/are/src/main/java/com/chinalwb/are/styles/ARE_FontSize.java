...
//	public static boolean S_CHECKED = true;
    ...
    private AREditText mToolbarEditText;
    ...
    public ARE_FontSize(LinearLayout linearLayout) {
    ...
    public void onFontSizeChange(int fontSize) {
        mIsChecked = true;
        mSize = fontSize;
        if (null != mEditText || null!=mToolbarEditText ) {
            EditText testEdtText=null;
            if (mEditText!=null) testEdtText = mEditText;
            else if (mToolbarEditText!=null)testEdtText = mToolbarEditText;
            int start = testEdtText.getSelectionStart();
            int end = testEdtText.getSelectionEnd();
            if (end >= start) {
                applyNewStyle(testEdtText.getEditableText(), start, end, mSize);
            }
        }
    }
    ...
    public void setToolbarEditText(AREditText mEditText) {
        mToolbarEditText = mEditText;
        if (mFontPickerWindow != null) mFontPickerWindow.setToolbarEditText(mEditText);
    }
}
